...
class create_recs_dataset(Dataset):
    def __init__(self, cfg, mode='train'):
        ...
        if mode == 'train':
            txt_path = osp(self.root, 'phrase_txts',
                           'trainval_mbr.txt' if cfg['dataset'] not in ['RefDIOR_RECS', 'RefCOCO_RECS']
                           else 'abla_splits/10/labeled.json')
        elif mode == 'val':
            txt_path = osp(self.root, 'phrase_txts',
                           'test_mbr.txt' if cfg['dataset'] not in ['RefDIOR_RECS', 'RefCOCO_RECS']
                           else 'val_ablation.json')
        else:
            raise NotImplementedError

        self.uni_ids, self.img_ids, self.mask_ids, self.bboxs, self.phrases = \
    ...
    def __getitem__(self, item):
        uni_id = self.uni_ids[item]

        img, mask = load_img_mask_file(need_bands=self.need_bands, img_dir=self.img_ids[item],
                                       mask_dir=self.mask_ids[item], convert_255_1=True)
        ...
        word_id, word_mask = self._tokenize(target['phrase'])

        mask = target.pop('ris_mask') # binary mask for ris
        img_mask = target.pop('mask') # binary mask for augmented img areas
        bbox = target.pop('bbox')

        if self.mode == 'train':
            return img, img_mask, word_id, word_mask, mask, bbox
        else:
            ...
            return img, img_mask, word_id, word_mask, orig_mask, orig_bbox, size, ratio, orig_size, dxdy, uni_id
...
class create_semi_recs_dataset(Dataset):
    def __init__(self, cfg, mode='train_l', split='1', nsample=None):
        ...
        if self.mode == 'train_l' and nsample is not None and nsample > len(self.uni_ids):
            ...
            self.bboxs *= resample_ratio
            self.bboxs = self.bboxs[:nsample]
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'val':
            ...
            if self.mode == 'train_l':
                ...
                word_id, word_mask = self._tokenize(target['phrase'])
                img_mask = target.pop('mask')  # binary mask for augmented img areas
                return img, img_s, img_mask, word_id, word_mask
...
#         if mode == 'train':
#             txt_path = osp(self.root, 'phrase_txts',
#                            'trainval.txt' if cfg['dataset'] not in ['RefDIOR_RECS', 'RefCOCO_RECS']
#                            else 'train.json')
#         elif mode == 'val':
#             txt_path = osp(self.root, 'phrase_txts',
#                            'test.txt' if cfg['dataset'] not in ['RefDIOR_RECS', 'RefCOCO_RECS']
#                            else 'val.json')
#         else:
#             raise NotImplementedError
#
#         self.uni_ids, self.img_ids, self.mask_ids, self.bboxs, self.phrases = \
#             load_id_phrase_mask_bbox_info(txt_path, self.img_suffix, self.mask_suffix)
#
#         self.transform = make_vlm_transforms(self.size, self.mode, cfg.get('strong_aug', False))
#
#     def __getitem__(self, item):
#         uni_id = self.uni_ids[item]
#
#         img = load_img_mask_file(need_bands=self.need_bands, img_dir=self.img_ids[item],
