...
from rsfm.utils import MLP, get_clones
...
class vg_decoder_wrapper(nn.Module):
    ...
    def forward(self, img_feat, mask, pos_embed, word_feat, word_mask):
        hs = self.decoder(img_feat, mask, pos_embed, word_feat, word_mask)
...
class MultiStageDecoderLayer(nn.Module):
    def __init__(self, d_model, dim_feedforward=2048, dropout=0.1,
        ...
        self.norm = get_clones(nn.LayerNorm(d_model), 3)
        self.dropout = get_clones(nn.Dropout(dropout), 3)
...
def build_vg_decoder(args):
    return vg_decoder_wrapper(args)


class MHAttentionRPE(nn.Module):
    ''' With relative position embedding '''
    def __init__(self, d_model, h, dropout=0.1, return_raw_attention=False, img_size=640, learnable_pos_embed=False):
        super().__init__()
        self.d_k = d_model // h
        self.h = h
        ...
        pos_index_offset = img_size // 32
        pos_x_range = pos_y_range = [-pos_index_offset, pos_index_offset]
    ...
    def forward(self, query, key, value, key_padding_mask=None):
        ...
        assert (HW == self.pos_index_offset ** 2) and (HW == tgt_len)
        img_mask = ~key_padding_mask.view(bs, self.pos_index_offset, self.pos_index_offset)
