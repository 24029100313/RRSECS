...
class PseudoQHead(nn.Module):
    def __init__(self,
                 ...
                 num_classes=1,
                 aux_loss=False,
                 trans_enc=False, **kwargs):
        super(PseudoQHead, self).__init__()
        ...
        self.cross_module = cross_module(cross_vis_dim=embedding_dim if trans_enc else in_channels[-1])
        ...
        self.vis_proj = nn.Linear(embedding_dim if trans_enc else in_channels[-1], embedding_dim)
    ...
    def forward(self, features, pos, text_feat, text_mask):
        ...
        tgt_src = self.reg_token.weight.unsqueeze(1).repeat(1, B, 1) # [1, B, embedding_dim]
        tgt_mask = torch.zeros((B, 1)).to(tgt_src.device).to(torch.bool) # [B, 1]
        text_feat, vis_feat = self.cross_module(vis_feat, text_feat, vis_mask, text_mask)
        ...
        vl_pos = self.vl_pos_embed.weight.unsqueeze(1).repeat(1, B, 1) # [1+L+HW/32*32, B, embedding_dim]
