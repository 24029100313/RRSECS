...
from .transformer import TransformerDecoder, TransformerDecoderLayer, TransformerEncoder, \
...
class VLTransformer(nn.Module):
    ...
    def encode(self, img_srcs, img_masks, img_pos_embeds,
        ...
        img_masks, img_srcs, img_pos_embeds = self.process_img_feat(img_srcs, img_masks, img_pos_embeds)
        lang_masks, lang_srcs, lang_pos_embeds = self.process_lang_feat(lang_srcs, lang_masks)
...
def build_vl_transformer(hidden_dim, nheads, enc_layers, dec_layers, dim_feedforward,
                         dropout, num_feature_levels, return_intermediate_dec):
...
class QueryEncoder(nn.Module):
    def __init__(self, num_queries_per_phrase, hidden_dim):
...
class MaskHeadSmallConv(nn.Module):
    ...
    def __init__(self, dim, fpn_dims, context_dim, num_classes):
        ...
        self.adapter1 = torch.nn.Conv2d(fpn_dims[2], inter_dims[1], 1)
        self.adapter2 = torch.nn.Conv2d(fpn_dims[1], inter_dims[2], 1)
        self.adapter3 = torch.nn.Conv2d(fpn_dims[0], inter_dims[3], 1)
