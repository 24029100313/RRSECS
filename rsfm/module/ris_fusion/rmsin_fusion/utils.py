...
def _conv_bn2(input_channel, output_channel, kernel_size=3, padding=1, stride=1, groups=1):
    res = nn.Sequential()
    res.add_module('conv1', nn.Conv2d(in_channels=input_channel, out_channels=output_channel, kernel_size=1, padding=0,
                                      padding_mode='zeros', stride=stride, groups=groups, bias=False))
    res.add_module('bn1', nn.BatchNorm2d(output_channel))
    res.add_module('conv2', nn.Conv2d(in_channels=input_channel, out_channels=output_channel, kernel_size=kernel_size,
                                      padding=padding, padding_mode='zeros', stride=stride, groups=groups, bias=False))
...
class RepBlock(nn.Module):
    def __init__(self, input_channel, output_channel, kernel_size=3, groups=1, stride=1):
        ...
        self.brb_3x3 = _conv_bn2(input_channel, output_channel, kernel_size=self.kernel_size, padding=self.padding,
                                 groups=groups)
        ...
        self.brb_3x3_2 = _conv_bn2(input_channel, output_channel, kernel_size=self.kernel_size, padding=self.padding,
                                   groups=groups)
...
class Attention(torch.nn.Module):
    def __init__(self, dim, img_shape, att_shape, key_dim=32, num_heads=8, attn_ratio=2):
        super().__init__()
...
class CrossScaleAttention(nn.Module):
    def __init__(self, dim, height, width):
        ...
        att_shape = sum([math.ceil(height / (i + 1)) ** 2 for i in range(3)])
    ...
    def forward(self, x):
        ...
        attn = attn.permute(0, 2, 1).contiguous().view(x0.shape[0], x0.shape[1], self.height, self.width)
...
class CIMBlock(nn.Module):
    def __init__(self, dim, channels, mlp_ratio=2, height=15, width=15):
        super().__init__()
        self.csa1 = Residual(CrossScaleAttention(dim, height, width))
        self.intra_ff = Residual(IntraFeedForward(channels, mlp_ratio))
        self.csa2 = Residual(CrossScaleAttention(dim, height, width))
...
class CIM(nn.Module):
    def __init__(self, dim, num_layers=1, channels=[128, 256, 512, 1024], downsample=1, height=15, width=15):
        super().__init__()
        ...
        self.block = nn.ModuleList([
            CIMBlock(self.hidden_dim, channels, height=height, width=width)
