...
class ViT_scalemae(VisionTransformer):
    ...
    def __init__(self, global_pool=False, out_indices=(2, 5, 8, 11), patch_norm=False,
                 interpolate_mode='bicubic', norm_cfg=dict(type='LN', eps=1e-6), **kwargs):
        super(ViT_scalemae, self).__init__(img_size=kwargs['img_size'],
                                           embed_dim=kwargs['embed_dim'],
                                           depth=kwargs['depth'],
                                           num_heads=kwargs['num_heads'],
                                           mlp_ratio=kwargs['mlp_ratio'],
                                           qkv_bias=kwargs['qkv_bias'],
                                           weight_init=None,  # Add
                                           )  # **kwargs
        self.img_size = kwargs['img_size']
        self.patch_size = self.patch_embed.patch_size[0]  # int
    ...
    def forward_features(self, x):
        ...
        input_res = np.full(B, 10)
        input_res = torch.tensor(input_res)
...
def vit_large_patch16_scalemae_timm(**kwargs):
    model = ViT_scalemae(embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4, qkv_bias=True,
                         out_indices=(7, 11, 15, 23), norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model