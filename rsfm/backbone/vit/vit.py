...
from mmcv.runner import (BaseModule, ModuleList, load_state_dict)
from torch.nn.modules.batchnorm import _BatchNorm
from torch.nn.modules.utils import _pair as to_2tuple
from .utils import PatchEmbed, TransformerEncoderLayer
        ...
        if self.vlf_ris:
            for i, out_indice in enumerate(self.out_indices):
                fusion = build_ris_fusion(vlf_ris, embed_dim, l_dim, num_heads_fusion[i], fusion_drop,
                                          size=img_size[0] // patch_size)
                setattr(self, f"fusion{out_indice}", fusion)

        if self.vlf_vg:
            for i, out_indice in enumerate(self.out_indices):
                fusion = build_vg_fusion(vlf_vg, embed_dim, l_dim, size=img_size[0] // patch_size)
                setattr(self, f"fusion{out_indice}", fusion)

        if self.vlf_ris == 'RMSIN':
            self.cim = CIM(dim=embed_dim * 4,
                           channels=[embed_dim] * 4,
                           height=img_size[0] // patch_size,
                           width=img_size[0] // patch_size)
