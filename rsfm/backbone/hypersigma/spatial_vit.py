...
def deform_PatchInputs_func(x, seg_patch_size):
    B, c, h, w = x.shape
    spatial_shapes = torch.as_tensor([h // seg_patch_size, w // seg_patch_size], dtype=torch.long, device=x.device)
    reference_points = get_reference_points([h // seg_patch_size, w // seg_patch_size], x.device)
...
class SpatViT(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_channels=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 interval=3, restart_regression=True, n_points=4):
        super().__init__()
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        self.in_channels = in_channels
        self.seg_patch_size = patch_size
    ...
    def forward_features(self, x):
        ...
        outputs = list(map(lambda x: x.permute(0, 2, 1).reshape(B, -1, Hp, Wp), outputs))
...
class vit_base_patch16_spatsigma(SpatViT):
    def __init__(self, **kwargs):
        super(vit_base_patch16_spatsigma, self).__init__(
...
class vit_large_patch16_spatsigma(SpatViT):
    def __init__(self, **kwargs):
        super(vit_large_patch16_spatsigma, self).__init__(
...
class vit_huge_patch16_spatsigma(SpatViT):
    def __init__(self, **kwargs):
        super(vit_huge_patch16_spatsigma, self).__init__(
