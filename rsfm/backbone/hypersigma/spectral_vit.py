...
class SampleAttention(nn.Module):
    def __init__(
        ...
        self.sampling_offsets = nn.Linear(all_head_dim, self.num_heads * n_points * 2)  # ????n point????????level??????????
    ...
    def forward(self, x, H, W, deform_inputs):
        ...
        offset_normalizer = torch.stack([input_spatial_shapes[1], input_spatial_shapes[0]])  # ??????????????xy

        sampling_locations = reference_points[:, None, :, None, :] \
                             + sampling_offsets / offset_normalizer[None, None, None, None, :]  # ????level??xy??????????
...
class SpecViT(nn.Module):
    ...
    def __init__(self, NUM_TOKENS=100, img_size=224, in_channels=3, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=None, init_values=None, with_cp=False,
                 use_abs_pos_emb=False, use_rel_pos_bias=False, out_indices=[3, 5, 7, 11],
                 interval=3, restart_regression=True, n_points=8):
        super().__init__()
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in self.out_indices:
                outputs.append(x)  # b, channels, embed_dim

        # outputs = list(map(lambda x: x.permute(0, 2, 1).reshape(B, -1, H, W), features))

        ops = [self.l1, self.l2, self.l3, self.l4]
        for i in range(len(ops)):
            outputs[i] = ops[i](outputs[i])
...
class vit_base_patch16_specsigma(SpecViT):
    def __init__(self, **kwargs):
        super(vit_base_patch16_specsigma, self).__init__(
            drop_path_rate=0.1, out_indices=[3, 5, 7, 11], embed_dim=768, depth=12, num_heads=12,
            mlp_ratio=4, qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
            with_cp=False, use_abs_pos_emb=False, interval=3, **kwargs)


class vit_large_patch16_specsigma(SpecViT):
    def __init__(self, **kwargs):
        super(vit_large_patch16_specsigma, self).__init__(
            drop_path_rate=0.1, out_indices=[7, 11, 15, 23], embed_dim=1024, depth=24, num_heads=16,
            mlp_ratio=4, qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
            with_cp=False, use_abs_pos_emb=False, interval=6, **kwargs)


class vit_huge_patch16_specsigma(SpecViT):
    def __init__(self, **kwargs):
        super(vit_huge_patch16_specsigma, self).__init__(
            drop_path_rate=0.1, out_indices=[10, 15, 20, 31], embed_dim=1280, depth=32, num_heads=16,
            mlp_ratio=4, qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
            with_cp=True, use_abs_pos_emb=False, interval=8, **kwargs)
