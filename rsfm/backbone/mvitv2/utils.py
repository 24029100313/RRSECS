...
def prepare_mvit_configs(depth, downscale_indices, pool_kvq_kernel, pool_kv_stride_adaptive):
    dim_mul, head_mul = torch.ones(depth + 1), torch.ones(depth + 1)
    for ind in downscale_indices:
        dim_mul[ind] = 2.0
        head_mul[ind] = 2.0
    ...
    if pool_kv_stride_adaptive is not None:
        ...
        for i in range(depth):
            ...
            pool_kv_strides.append([i] + _stride_kv)

    for i in range(len(pool_kv_strides)):
        stride_kv[pool_kv_strides[i][0]] = pool_kv_strides[i][1:]
        pool_kv[pool_kv_strides[i][0]] = pool_kvq_kernel
