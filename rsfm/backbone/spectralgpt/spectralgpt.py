...
from ..satmae_pp.pos_embed import interpolate_pos_embed
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.patch_embed = PatchEmbed(
            img_size, patch_size, in_chans, embed_dim, in_channels, t_patch_size
        )
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            ...
            for k in ['pos_embed', 'patch_embed.proj.weight', 'patch_embed.proj.bias',
                      'head.weight', 'head.bias', 'pos_embed_spatial']:
                if k in checkpoint and checkpoint[k].shape != init_state_dict[k].shape:
                    del checkpoint[k]
            interpolate_pos_embed(self, checkpoint)
...
class vit_base_patch16_spectralgpt(VisionTransformer):
    def __init__(self, **kwargs):
        super(vit_base_patch16_spectralgpt, self).__init__(
            in_chans=1, patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
            t_patch_size=3, norm_layer=partial(nn.LayerNorm, eps=1e-6), with_cp=False, **kwargs)
