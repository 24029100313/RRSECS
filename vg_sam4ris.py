...
#              save_path='/root/lxq/RefDIOR_results/RSVG/wosa/test/masks/RefDIOR_VG.swin_tiny.None.LQVGHead.4xb8.img512.ep50.preimagenet.best81.86_predictions/sam_vit_h',
#              type='vit_h')



encs = ['swin', 'convnext', 'vmamba']
path = '/root/lxq/RefDIOR_results/RSVG/wosa/test/jsons'
needs = []
for enc in encs:
    for file in os.listdir(path):
        if 'LPVA' in file and enc in file:
            needs.append(path + '/' + file)

for need in needs:
    print(need)
    for model in ['vit_b', 'vit_l', 'vit_h']:
        det_sam_mask(vg_json=need,
                     img_path='/root/lxq/RSFM_RIS_Datasets/Optical/RefDIOR/images',
                     save_path=need.replace('jsons', 'masks').replace('.json', '') + '/sam_' + model,
                     type=model)
